...
4. **SRV-001 follow-up** *(small)*: Refine the `except ValueError` handler at cli.py:476-481 to distinguish quality-parsing errors from URL-format errors (e.g., inspect the error message or use structured error types), so the misleading "Invalid URL format" message is not shown when the URL is valid.
