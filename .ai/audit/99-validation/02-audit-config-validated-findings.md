---
name: 02-audit-config-validated-findings
description: Validated findings for Configuration & Settings Models audit
agent: validator
alwaysApply: false
---
...
| **Severity** | LOW |
| **Type** | DOC-UPDATE |
| **Affected Modules** | `src/vkdownloader/config.py`, `docs/11-guides/configuration.md` |
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** Code correctly uses default 10000 (conservative 10KB/s). Documentation incorrectly states 100000. This is a documentation error, not a spec deviation.
> - **Evidence:** Tests pass with 10000, ruff check and mypy pass on config.py.
...
**Description:** The `Settings` model defines two browser-stealth timing fields, `browser_pre_interaction_wait` (default 5) and `browser_post_interaction_wait` (default 8), which are actively consumed by the extractor (`services/extractor.py:212` and `services/extractor.py:214`). Neither field appears anywhere in `configuration.md` (neither in the settings table nor in any section). A user reading the configuration guide cannot discover or tune these values, and there is no `VKDOWNLOADER_*` env var mapping documented for them.

**Evidence:**
- `config.py:55-66` — both fields defined with `VKDOWNLOADER_*` env mapping via `env_prefix`.
- `services/extractor.py:212` — `await asyncio.sleep(self.settings.browser_pre_interaction_wait)`
- `services/extractor.py:214` — `await asyncio.sleep(self.settings.browser_post_interaction_wait)`
- `configuration.md` — full text search shows no occurrence of `browser_pre_interaction_wait` or `browser_post_interaction_wait`.

> **Validation Note:**
> - **Action:** validated
> - **Detail:** Fields are present in config.py and actively consumed in extractor.py. Documentation omits these fields entirely.
> - **Evidence:** extractor.py lines 212-214 confirm active usage.
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** Code inspection confirms Settings() is called without try/except for ValidationError at cli.py lines 392 and 527. Both download and batch_download commands lack configuration error handling.
> - **Evidence:** cli.py shows direct Settings() calls in both command functions with no ValidationError exception handling.
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** This is a documented limitation of pydantic-settings v2. The code explicitly documents this at config.py:18-20. The extra='forbid' only applies to kwargs, not environment variables.
> - **Evidence:** config.py docstring acknowledges the limitation; test confirms unknown kwargs are rejected while env vars silently pass.
...
- `downloader.py:179-180` — `max_retries` used in `_build_ytdlp_options` without batch-only guard
- `segment_downloader.py:734,757` — `max_retries` passed to DownloadPolicy for segment downloads

> **Validation Note:**
> - **Action:** validated
> - **Detail:** Code inspection confirms max_retries is used in both single download (via _build_ytdlp_options and HLSDownloader) and batch download paths. The documentation incorrectly limits scope to batch processing.
> - **Evidence:** downloader.py lines 179-180 and segment_downloader.py lines 734, 757 show usage in shared download paths.
...
|--------|-------|---------|
| Validated (unchanged) | 5 | All findings are valid and current |
| Reclassified | 0 | — |
| Merged | 0 | — |
| Rejected | 0 | — |
...
- `uv run pytest tests/test_config.py` — All 12 tests passed.
- `uv run ruff check src/vkdownloader/config.py` — All checks passed.
- `uv run mypy src/vkdownloader/config.py` — No errors (only unused section warning for tests.*).

### Architectural Impact Assessment

All findings are **DOC-UPDATE** or **RUNTIME-ERROR** type with minimal architectural impact:

- **CFG-001, CFG-002, CFG-005** are documentation-only fixes that improve user experience without changing code behavior.
- **CFG-003, CFG-004** involve runtime error handling improvements. CFG-003 would improve user-facing error messages. CFG-004 acknowledges a pydantic-settings limitation and recommends either warning logs or documentation.

### Rollout Safety

No rollout risks identified. All recommendations:
- Are backward compatible
- Do not require database migrations
- Can be implemented independently
- Have low coupling to other system components