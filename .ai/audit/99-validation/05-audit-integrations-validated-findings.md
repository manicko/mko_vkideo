...
3. **INT-003** (MEDIUM, mandatory): In `src/vkdownloader/services/downloader.py`, address the inability to cancel the yt-dlp executor thread. Preferred: restructure `_download_with_ytdlp` to use `asyncio.create_subprocess_exec` (mirroring `download_with_ffmpeg` at `downloader.py:333`). Minimum: document the zombie-thread limitation in the code and limitations doc.
