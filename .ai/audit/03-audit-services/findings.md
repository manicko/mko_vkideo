...
**Recommendation:** Apply the same wait-and-retry loop (`_retry_429_with_backoff`, honoring
`Retry-After` and jitter) inside the parallel path, or uniformly route failed segments through a
shared retry worker. The `URLBackoffCoordinator` should coordinate, not substitute for, retries.
...
### SRV-007: Unreachable `max_retries_exceeded` branch in `download_with_ytdlp_with_resume_fallback`
...
**Description:** The retry loop condition is `while retry_count <= MAX_RESUME_RETRIES:` so
`retry_count` reaches at most `MAX_RESUME_RETRIES`. Inside the loop the refresh branch is guarded by
`if retry_count <= MAX_RESUME_RETRIES:` (always true within the loop), making the `else:
logger.error("max_retries_exceeded")` unreachable. The intended "max retries exceeded" signal never
fires; instead the function silently returns `None` after the loop.

**Evidence:**
`downloader.py:276` (`while retry_count <= MAX_RESUME_RETRIES:`), `downloader.py:297`
(`if retry_count <= MAX_RESUME_RETRIES:`), `downloader.py:328-330` (unreachable `else`).

**Recommendation:** Change the guard to `if retry_count < MAX_RESUME_RETRIES:` (or restructure the
loop) so the exhaustion path is reachable and logged. Effort: trivial. Priority: recommended.
...
**Recommendation:** Confirm intent. If unused, remove it; if it is meant to gate downloads before
starting, wire a caller. Effort: trivial. Priority: recommended.
...
**Recommendation:** Either adopt `DownloadResult` as the real return type of `perform_download` (and
populate it), or remove the unused models to avoid implying behavior that does not exist. Effort: small.
Priority: recommended.
...
**Recommendation:** Either implement file-cookie loading or fail fast / warn clearly when
`CookieSource.FILE` is selected, so users are not silently given unauthenticated behavior. Effort: small.
Priority: recommended.
...
- SRV-007 (unreachable max-retries-exceeded branch)
- SRV-008 (unused `is_paused`)
